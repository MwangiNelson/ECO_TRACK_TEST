...
import firebase_admin  # Ensure firebase_admin is imported properly
...
def component():
    ...
    height = tab1col1.number_input(
        "Height", 0, 251, value=None, placeholder="160", help="in cm"
    )
    weight = tab1col2.number_input(
        "Weight", 0, 250, value=None, placeholder="75", help="in kg"
    )
    ...
    body_type = (
        ...
        if (calculation < 18.5)
            ...
            "normal"
            if ((calculation >= 18.5) and (calculation < 25))
            else (
                "overweight"
                if ((calculation >= 25) and (calculation < 30))
                else "obese"
            )
        )
    )
    ...
    social = tab1.selectbox(
        "Social Activity",
        ["never", "often", "sometimes"],
        help="How often do you go out?",
    )
    ...
    waste_bag = tab3.selectbox(
        "What is the size of your waste bag?",
        ["small", "medium", "large", "extra large"],
    )
    waste_count = tab3.slider(
        "How many waste bags do you trash out in a week?", 0, 10, 0
    )
    recycle = tab3.multiselect(
        "Do you recycle any materials below?", ["Plastic", "Paper", "Metal", "Glass"]
    )
    ...
    daily_tv_pc = tab4.slider(
        "How many hours a day do you spend in front of your PC/TV?", 0, 24, 0
    )
    internet_daily = tab4.slider(
        "What is your daily internet usage in hours?", 0, 24, 0
    )
...
tab_result.markdown(
    f"""You owe nature <b>{tree_count}</b> tree{'s' if tree_count > 1 else ''} monthly. <br> {f"<a href='https://www.tema.org.tr/en/homepage' id = 'button-17' class='button-17' role='button'> 🌳 Proceed to offset 🌳</a>" if tree_count > 0 else ""}""",
